\begin{table}[t]
\centering
\caption{Table 4 — Representation readout ablation}
...
\bottomrule
\end{tabular}
\footnotetext[1]{Δ AUPRC = pre-3h minus post-128 on paired rows.}
\footnotetext[2]{Small-LI Δ AUPRC uses mean ± sample SD over per-seed paired deltas when available.}
\footnotetext[3]{emb198 scout omitted from main table (diagnostic-only; see contrastive appendix if included).}
\end{table}
