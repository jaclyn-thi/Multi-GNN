...
7. **Optional offline feature-KNN negative filtering (Jun 2026)**
   ...
   - **Jun 2026 ablation:** random 8192-neighbor sampling + exclusion **did not help** (sparse pool overlap); soft positives **hurt** — see [`results-archive.md`](results-archive.md#feature-knn-small-hi)
...
**Current status**: Largely implemented (Jun 2026).
...
1. **New command-line arguments**
   ...
   - `--contrastive_accum_steps`
   - `--contrastive_memory_bank_size`
   - `--contrast_projection_head`, `--contrast_projection_hidden`, `--contrast_projection_dim`
   - `--checkpoint_policy {last,best}`
   - `--loader_num_workers`
   - full morphology CLI surface (see `README.md` and morphology plan)
...
- We **did** add GraphCL-style projection head (not in original six-phase plan)
...
5. **Downstream adaptation & evaluation** (Phase 5)
   - Original plan included per-epoch AML during contrastive; **replaced** by Papagei-style extract + linear probe (see Phase 5a/5b/5c)
   - Status update: **done** on Small-HI — SSL leaderboard led by **M1b+clustering+projection** (0.929 test AUROC); contrastive+proj 0.927; supervised in-GNN ~0.97 remains ceiling

6. **Configuration & Documentation** (Phase 6)
   - Status update: CLI, README runbook, and design notes largely complete (Jun 2026)
...
- **Jun 2026 status:** Small-HI benchmark matrix largely complete. **Relax grid (Jun 11):** asym+8192 negs **0.930** AUROC; sym+1024 **0.929** AUROC / **0.222** F1; asym@16384 confound shows batch size drives most F1 lift. Queue sweep improves the asym + projection recipe by disabling the queue: **0.951 AUROC**, **0.233 F1**. Follow-up runs show larger negatives and filtered queues do not improve metrics; `same_pair` false-negative filtering is the leading replicated F1/recall candidate. Multi-positive InfoNCE has not beaten exclusion-only filtering. **Feature-KNN negative exclusion** (random 8192 negs) also did not help. See [`projection-head-ablation-jun2026.md`](archive/projection-head-ablation-jun2026.md) and [`results.md`](results.md). MAE expert ablation done (0.898, below MSE).
- **Best SSL recipe (development):** the numbers in this bullet are a Jun 2026 snapshot and may lag the latest batch. **Canonical current recipes/results:** [`current_protocol_recent_runs_summary.md`](current_protocol_recent_runs_summary.md) and [`results.md` § Recommended configs](results.md#recommended-configs-jun-2026). Snapshot for context: contrastive+projection, 8192 negatives, `queue=0`, `bs=8192`, `accum=4` (**0.951** / **0.233**); + `--false_neg_filter_mode same_pair` as an F1/recall candidate (mean **0.942** / **0.236** over three seeds); morph (asym) clustering+proj (**0.929** / 0.156). README benchmark table is a sanity-check log, not formal recorded experiments.
