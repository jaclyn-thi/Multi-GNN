#!/usr/bin/env python3
"""GBT+TF adaptive stdfloor frozen R198 validation-only evaluation.

Subcommands:
  preflight  — verify training integrity + checkpoints + baseline comparability
  extract    — one GPU cell: GBT+TF step × target full-subgraph R198 train/val
  probe      — one CPU job per target: PaperStyleMLP on GBT_TF@1500 + @3000
  finalize   — tables vs GBT-stdfloor / EXPERT_ONLY / LONG / projection baselines

Never retrains encoders. Never loads/scores test. Never uses recovery checkpoints.
Uses direct encoder R198 (not GBT projection). Training auth via revalidation path.
"""
...
from gbt_tf_adaptive_stdfloor_frozen_eval import (  # noqa: E402
    BASELINE_ARMS,
...
def verify_gbt_checkpoint(arm: str) -> Dict[str, Any]:
    ...
    gates = {
        ...
        "ckpt_root_is_train_tree": str(recipe.get("ckpt_root") or "") == CKPT_ROOT
        or str(recipe.get("ckpt_root") or "").endswith(CKPT_ROOT),
...
def verify_training_aggregate() -> Dict[str, Any]:
    """Authorize via original PASS or C_shape logging-gate revalidation."""
    auth = verify_training_authorized_for_frozen_eval(ROOT / TRAIN_RESULT_ROOT)
    ...
    seed_ok = {}
    ok = bool(auth.get("ok")) and bool(auth.get("authorized_for_frozen_eval"))
    if seed_blob:
    ...
    init_ok = (not init_sha) or (init_sha == INIT_SHA256)
    if not init_ok:
        ok = False
    return {
        "ok": ok,
        "path": str(agg_path),
        "classification": auth.get("revalidation_classification")
        or agg.get("classification"),
        "authorized_for_frozen_eval": bool(auth.get("authorized_for_frozen_eval")),
        "acceptance_path": auth.get("acceptance_path"),
        "authorization": auth,
        ...
        "alpha_unfrozen_at": agg.get("alpha_unfrozen_at"),
    }
...
def verify_cli_guards() -> Dict[str, Any]:
    ...
    help_cmds = [
        [sys.executable, str(ROOT / "scripts/run_gbt_tf_adaptive_stdfloor_frozen_eval.py"), "--help"],
        [sys.executable, str(ROOT / "scripts/run_gbt_tf_adaptive_stdfloor_frozen_eval.py"), "preflight", "--help"],
        [sys.executable, str(ROOT / "scripts/run_gbt_tf_adaptive_stdfloor_frozen_eval.py"), "extract", "--help"],
        [sys.executable, str(ROOT / "scripts/run_gbt_tf_adaptive_stdfloor_frozen_eval.py"), "probe", "--help"],
        [sys.executable, str(ROOT / "scripts/run_gbt_tf_adaptive_stdfloor_frozen_eval.py"), "finalize", "--help"],
    ]
    help_ok = {}
...
def build_baseline_comparability_card() -> Dict[str, Any]:
    ...
    return {
        ...
        "historical_adaptive_long_1500": {
            "included": "ADAPTIVE_LONG@1500" in BASELINE_ARMS,
            "note": (
                "Included only when protocol-comparable under the same frozen-eval card; "
                "clearly labeled matched_step_1500 vs unmatched if absent."
            ),
        },
    }
...
def cmd_preflight(_: argparse.Namespace) -> int:
    ...
    report = {
        ...
        "phase": "gbt_tf_adaptive_stdfloor_1e4_frozen_eval",
        "objective_id": OBJECTIVE_ID,
        "gbt_std_floor": GBT_STD_FLOOR,
        ...
        "cells": cells,
        "cell_array_mapping": {
            "0-2": "GBT_TF_ADAPTIVE_1500 × (Small-HI, SAML-D, Small-LI)",
            "3-5": "GBT_TF_ADAPTIVE_3000 × targets",
        },
...
def run_extract_cell(arm: str, target: str) -> Dict[str, Any]:
    ...
    if not bool(recipe.get("tfmoe_enabled")):
        raise RuntimeError("expected tfmoe_enabled=True for GBT+TF arm")
    ...
    if bool(recipe.get("infonce_enabled")):
        raise RuntimeError("infonce must be disabled for GBT+TF arm")
    ...
    meta = {
        ...
        "skip_test_eval": True,
        "extractor": "run_gbt_tf_adaptive_stdfloor_frozen_eval.run_extract_cell",
...
def _alpha_beta_from_ckpt(arm: str) -> Dict[str, Any]:
    ...
    with torch.no_grad():
        ...
        eff = ab.effective_weights("adaptive")
    return {
        "arm": arm,
        "checkpoint_step": CHECKPOINT_STEP[arm],
        "alpha": float(alpha),
        ...
        "alpha_unfrozen_at": (blob.get("extra") or {}).get("alpha_unfrozen_at"),
    }
...
def _step_row_at(step: int) -> Optional[Dict[str, Any]]:
    ...
    with path.open(encoding="utf-8") as f:
        for line in f:
            row = json.loads(line)
            s = int(row.get("step", row.get("global_step", -1)))
            if s == step:
                hit = row
    return hit
...
def _realized_shares_from_step(row: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    w_gbt = float(row.get("weighted_gbt", alpha * float(row.get("L_gbt_norm", 0))))
    # Prefer explicit adaptive shares: α and (1-α)β_i
    shares = {
        "alpha": alpha,
        "beta": betas,
        "share_gbt": alpha,
        "share_tf": [(1.0 - alpha) * b for b in betas],
        "weighted_gbt": float(row.get("weighted_gbt")),
        "weighted_tf": [float(row.get(f"weighted_tf_{i}")) for i in range(3)],
        "L_gbt_norm": float(row.get("L_gbt_norm")),
        "L_tf_norm": [float(row.get(f"L_tf_norm_{i}")) for i in range(3)],
        "step": int(row.get("step", row.get("global_step", -1))),
    }
...
def _baseline_auprc(baselines: Dict[str, Dict[str, Dict[str, Any]]], label: str, target: str) -> float:
...
def build_scientific_answers(
    ...
    ab_by_arm: Dict[str, Dict[str, Any]],
    shares_by_step: Dict[int, Dict[str, Any]],
) -> Dict[str, Any]:
    """Answer Q1–Q10 from metrics/weights without causal gradient claims."""
    stronger = {}
    for target in TARGETS:
        a15 = _auprc(by, "GBT_TF_ADAPTIVE_1500", target)
        a30 = _auprc(by, "GBT_TF_ADAPTIVE_3000", target)
        stronger[target] = {
            "stronger_checkpoint": "1500" if a15 >= a30 else "3000",
            "AUPRC_1500": a15,
            "AUPRC_3000": a30,
            "delta_3000_minus_1500": a30 - a15,
        }

    def deltas(arm: str, label: str) -> Dict[str, float]:
        return {
            t: _auprc(by, arm, t) - _baseline_auprc(baselines, label, t) for t in TARGETS
        }

    q1 = {
        "question": "Does GBT+TF improve over standalone GBT?",
        "matched_step_1500_delta_AUPRC": deltas("GBT_TF_ADAPTIVE_1500", "GBT_STDFLOOR@1500"),
        "matched_step_3000_delta_AUPRC": deltas("GBT_TF_ADAPTIVE_3000", "GBT_STDFLOOR@3000"),
    }
    q1["verdict"] = {
        "at_1500": all(v > 0 for v in q1["matched_step_1500_delta_AUPRC"].values()),
        "at_3000": all(v > 0 for v in q1["matched_step_3000_delta_AUPRC"].values()),
        "any_target_improves_1500": any(v > 0 for v in q1["matched_step_1500_delta_AUPRC"].values()),
        "any_target_improves_3000": any(v > 0 for v in q1["matched_step_3000_delta_AUPRC"].values()),
    }

    q2 = {
        "question": "Does it match or exceed EXPERT_ONLY?",
        "match_kind": "matched_step_3000",
        "delta_AUPRC_vs_EXPERT_ONLY@3000": deltas("GBT_TF_ADAPTIVE_3000", "EXPERT_ONLY@3000"),
    }
    q2["verdict"] = {
        "matches_or_exceeds_all": all(v >= 0 for v in q2["delta_AUPRC_vs_EXPERT_ONLY@3000"].values()),
        "exceeds_any": any(v > 0 for v in q2["delta_AUPRC_vs_EXPERT_ONLY@3000"].values()),
    }

    q3 = {
        "question": "Does it improve over identity-InfoNCE adaptive LONG?",
        "match_kind": "matched_step_3000",
        "delta_AUPRC_vs_ADAPTIVE_LONG@3000": deltas("GBT_TF_ADAPTIVE_3000", "ADAPTIVE_LONG@3000"),
        "historical_1500_if_comparable": {
            "match_kind": BASELINE_MATCH_KIND.get("ADAPTIVE_LONG@1500"),
            "delta_AUPRC": deltas("GBT_TF_ADAPTIVE_1500", "ADAPTIVE_LONG@1500")
            if "ADAPTIVE_LONG@1500" in baselines
            else None,
        },
    }
    q3["verdict"] = {
        "improves_all_at_3000": all(
            v > 0 for v in q3["delta_AUPRC_vs_ADAPTIVE_LONG@3000"].values()
        ),
        "improves_any_at_3000": any(
            v > 0 for v in q3["delta_AUPRC_vs_ADAPTIVE_LONG@3000"].values()
        ),
    }

    q4 = {
        "question": "Does it beat projection-on adaptive?",
        "match_kind": "matched_step_3000",
        "delta_AUPRC_vs_PROJECTION_ON_ADAPTIVE@3000": deltas(
            "GBT_TF_ADAPTIVE_3000", "PROJECTION_ON_ADAPTIVE@3000"
        ),
    }
    q4["verdict"] = {
        "beats_all": all(v > 0 for v in q4["delta_AUPRC_vs_PROJECTION_ON_ADAPTIVE@3000"].values()),
        "beats_any": any(v > 0 for v in q4["delta_AUPRC_vs_PROJECTION_ON_ADAPTIVE@3000"].values()),
    }

    # Complementarity: improves vs GBT-only AND vs EXPERT_ONLY on some target
    # without claiming gradient compatibility.
    comp = {}
    for t in TARGETS:
        d_gbt = _auprc(by, "GBT_TF_ADAPTIVE_3000", t) - _baseline_auprc(
            baselines, "GBT_STDFLOOR@3000", t
        )
        d_exp = _auprc(by, "GBT_TF_ADAPTIVE_3000", t) - _baseline_auprc(
            baselines, "EXPERT_ONLY@3000", t
        )
        comp[t] = {
            "delta_vs_GBT_STDFLOOR@3000": d_gbt,
            "delta_vs_EXPERT_ONLY@3000": d_exp,
            "metric_complementarity_hint": d_gbt > 0 and d_exp > 0,
        }
    q5 = {
        "question": "Is there evidence of objective complementarity?",
        "note": (
            "Metric-level hint only: positive Δ vs both standalone GBT and EXPERT_ONLY. "
            "Do not infer causal gradient compatibility from weights or metrics alone."
        ),
        "per_target": comp,
        "verdict": any(v["metric_complementarity_hint"] for v in comp.values()),
    }

    ab1500 = ab_by_arm["GBT_TF_ADAPTIVE_1500"]
    ab3000 = ab_by_arm["GBT_TF_ADAPTIVE_3000"]
    q6 = {
        "question": "What alpha/beta and realized shares were learned?",
        "checkpoint_1500": ab1500,
        "checkpoint_3000": ab3000,
        "step_log_shares": shares_by_step,
    }

    # InfoNCE adaptive historically pushed α→0; here α≈0.875 means GBT prioritized.
    q7 = {
        "question": "Was GBT deprioritized like InfoNCE?",
        "alpha_1500": ab1500["alpha"],
        "alpha_3000": ab3000["alpha"],
        "init_alpha": 0.6,
        "verdict": {
            "gbt_deprioritized_like_infonce": ab3000["alpha"] < 0.25,
            "gbt_upweighted_from_init": ab3000["alpha"] > 0.6,
            "interpretation": (
                "α→0 would resemble InfoNCE deprioritization; observed α stays high → GBT retained."
                if ab3000["alpha"] >= 0.5
                else "α fell substantially; GBT share reduced (not necessarily InfoNCE-like)."
            ),
        },
    ...
    q8 = {
        "question": "Which checkpoint is stronger per target?",
        "per_target": stronger,
    }

    saml = stronger["SAML-D"]
    q9 = {
        "question": "Does SAML-D peak earlier again?",
        "SAML-D": saml,
        "verdict": saml["stronger_checkpoint"] == "1500",
    }

    # Q10: warrant later no-update GBT-vs-TF gradient diagnostic?
    # Only suggest if metrics look complementary OR weights look contradictory to metrics.
    warrant = bool(q5["verdict"]) or (
        ab3000["alpha"] > 0.8
        and not all(v > 0 for v in q1["matched_step_3000_delta_AUPRC"].values())
    )
    q10 = {
        "question": "Is a later no-update GBT-vs-TF gradient diagnostic warranted?",
        "verdict": warrant,
        "rationale": (
            "Metric complementarity hint and/or high-α without GBT-vs-baseline gains "
            "motivate a *future* no-update gradient diagnostic; not submitted now."
            if warrant
            else "Current metrics/weights do not clearly motivate an immediate gradient diagnostic."
        ),
        "submitted_now": False,
    }

    return {
        "Q1_vs_standalone_GBT": q1,
        "Q2_vs_EXPERT_ONLY": q2,
        "Q3_vs_ADAPTIVE_LONG": q3,
        "Q4_vs_PROJECTION_ON": q4,
        "Q5_complementarity": q5,
        "Q6_alpha_beta_shares": q6,
        "Q7_gbt_deprioritized": q7,
        "Q8_stronger_checkpoint": q8,
        "Q9_samld_peaks_early": q9,
        "Q10_gradient_diagnostic_warranted": q10,
    }
...
def cmd_finalize(_: argparse.Namespace) -> int:
            ...
            delta_rows.append(
                {
                    "GBT_TF_arm": arm,
                    "Target": target,
                    "GBT_TF_AUPRC": c["validation_auprc"],
                    "Baseline": label,
    ...
    gbt_step_delta = []
    for target in TARGETS:
        c1500 = by[("GBT_TF_ADAPTIVE_1500", target)]
        c3000 = by[("GBT_TF_ADAPTIVE_3000", target)]
    ...
    ab_by_arm = {arm: _alpha_beta_from_ckpt(arm) for arm in TRAIN_ARMS}
    # steps.jsonl uses 0-based completed index; checkpoint N ↔ logged step N-1
    shares_by_step = {
        1500: _realized_shares_from_step(_step_row_at(1499)),
        3000: _realized_shares_from_step(_step_row_at(2999)),
    }
    write_json(result_dir() / "adaptive_weights.json", {
        "checkpoints": ab_by_arm,
        "step_log_shares": shares_by_step,
    })
    answers = build_scientific_answers(by, baselines, ab_by_arm, shares_by_step)
    write_json(result_dir() / "scientific_answers.json", answers)

    write_json(result_dir() / "cells" / "gbt_tf_adap_table.json", table_rows)
    write_json(result_dir() / "cells" / "deltas_vs_baselines.json", delta_rows)
    write_json(result_dir() / "cells" / "gbt_tf_1500_vs_3000.json", gbt_step_delta)
    ...
    payload = {
        ...
        "adaptive_weights": {"checkpoints": ab_by_arm, "step_log_shares": shares_by_step},
        "baseline_comparability": comparability,
        "probe_protocol": PROBE,
        "table": table_rows,
        "deltas_vs_baselines": delta_rows,
        "gbt_tf_1500_vs_3000": gbt_step_delta,
        "baselines_reused_not_reextracted": True,
        "test_data_loaded_or_scored": False,
        "encoder_retrained": False,
        "no_recovery_checkpoint_eval": True,
        "no_gradient_diagnostic_submitted": True,
    }
    ...
    lines += ["", "## Adaptive α/β (checkpoint)", ""]
    for arm in TRAIN_ARMS:
        ab = ab_by_arm[arm]
        lines.append(
            f"- **{arm}**: α={ab['alpha']:.4f} β={ab['beta']} "
            f"w_gbt={ab['w_gbt']:.4f} w_tf={ab['w_tf']}"
        )

    lines += ["", "## Scientific answers (Q1–Q10)", ""]
    ...
    for d in delta_rows:
        lines.append(
            f"- **{d['GBT_TF_arm']} / {d['Target']} vs {d['Baseline']}** "
            f"[{d['match_kind']}]: Δ={d['delta_AUPRC']:+.4f} "
    ...
    lines += [
        ...
        "no gradient diagnostic submitted; recovery checkpoints never evaluated.",
        "",
    ]
