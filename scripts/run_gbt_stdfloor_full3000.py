#!/usr/bin/env python3
"""Fresh 3000-step MIXED_3DOMAIN_GBT_STDFLOOR_1E4_FULL3000_SEED2 training.

Fresh from Phase-3 shared init with std_floor=1e-4. Does not resume recovery or
the failed official full3000. No extraction/probes/test in this job.
"""
...
from graph_barlow_twins_r198 import (  # noqa: E402
    ...
    STDFLOOR_FULL3000_JSON,
    STDFLOOR_FULL3000_NOTE,
...
NOTE_PATH = ROOT / STDFLOOR_FULL3000_NOTE
TWIN_JSON = ROOT / STDFLOOR_FULL3000_JSON
...
def refuse_nonempty_run(out_dir: Path, ckpt_dir: Path) -> None:
    ...
    forbidden_sub = (
        ...
        "graph_barlow_twins_full3000_seed2",  # failed official eps run
    )
...
def should_console_log(completed: int) -> bool:
    ...
    if completed in (300, 600):
        return True
    if 500 <= completed <= 700 and completed % 10 == 0:
        return True
...
def cpu_reload_validate(ckpt_path: Path, *, expect_step: int, model_ref: nn.Module) -> Dict[str, Any]:
    ...
    with torch.no_grad():
        for p in model_cpu.parameters():
            p.normal_()
    loaded = load_gbt_checkpoint(ckpt_path, model_cpu, accepted_objective_ids=[OBJECTIVE_ID_STDFLOOR_1E4])
    ok = (
        loaded.get("objective_id") == OBJECTIVE_ID_STDFLOOR_1E4
...
def run_full3000() -> Dict[str, Any]:
    ...
    # Hard path isolation from recovery + failed official full3000.
    for forbidden in (
        ...
        if out_dir.resolve() == forbidden.resolve() or ckpt_dir.resolve() == forbidden.resolve():
            raise RuntimeError(f"stdfloor full3000 path collides with historical tree: {forbidden}")
        if forbidden in out_dir.resolve().parents or forbidden in ckpt_dir.resolve().parents:
            raise RuntimeError(f"stdfloor full3000 nested under historical tree: {forbidden}")
    ...
    unique = f"gbt_stdfloor_full3000_seed{SEED}"
    ns_by: Dict[str, Any] = {}
    ...
    try:
        with open(jsonl_path, "w", encoding="utf-8") as jsonl:
            for si in range(total_steps):
                ...
                if (
                    completed <= 30
                    or completed % 10 == 0
                    or completed in ckpt_steps
                    or 500 <= completed <= 700
                ):
                    jsonl.flush()
                rows.append(row)
                ...
                if should_console_log(completed):
                    logging.info(
                        "GBT stdfloor full step %s/%s domain=%s L=%.4f enc_g=%.3f lr=%.3e Δθ=%.3e",
                        completed,
                        total_steps,
                        domain,
                ...
                if completed in ckpt_steps or completed % ROLLING_EVERY == 0:
                    payload = build_checkpoint_payload(
                        ...
                        extra={
                            ...
                            "mode": "stdfloor_full3000",
                            "arm_label": STDFLOOR_FULL3000_ARM,
                            "resume_exact_verified": False,
        ...
        logging.exception("GBT stdfloor full3000 failed")
        write_json(
    ...
    matching_vs_long = {
        d: compare_vs_long(
            ...
            label_local=STDFLOOR_FULL3000_ARM,
        )
        for d in domains
    }
    ...
    gates = {
        ...
        "resume_exact_not_claimed": True,  # NeighborLoader resume not bit-exact verified
    ...
    result = {
        "title": "GBT stdfloor MIXED_3DOMAIN full 3000-step training integrity",
    ...
    lines = [
        ...
        f"- resume_exact_verified: false (fresh LONG stream match used instead)",
        "",
        "## Gates",
        "```",
